...
{#if visible}
	<Select.Root type="single" value={currentValue} onValueChange={(v) => v && setProvider(v)}>
		<Select.Trigger
			...
			class="ml-1.5 inline-flex h-8 items-center gap-1.5 px-1 text-xs font-semibold text-gray-600 transition-colors hover:text-gray-900 focus:outline-none dark:text-gray-300 dark:hover:text-gray-100 sm:h-7"
		>
