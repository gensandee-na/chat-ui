...
/**
 * Shared loader for the root layout and any sub-layout that needs the same
 * hydration (e.g. `embed/+layout.ts`). See `src/lib/load/conversation.ts`
 * for the rationale on the destructured-args pattern.
 */
export async function loadLayout(args: LayoutLoadArgs) {
	const { depends, fetch, url } = args;
